        ...
        <div className="flex-1 bg-white flex flex-col justify-between">
          ...
          <div className="h-[350px] overflow-scroll p-5 flex flex-col gap-5">
            {[...Array(10)].map((_, index) => (
              <div
                key={index}
                className={`w-1/2 ${index % 2 === 1 ? 'self-end text-right' : ''}`}
              >
                <p>Lorem ipsum dolor sit amet</p>
                <span className="text-xs bg-yellow-300/25 p-0.5 rounded">1 hour ago</span>
              </div>
            ))}
          </div>
          <div className="border-t-2 border-yellow-300/50 h-15 flex items-center justify-between">
            ...
            <button className="flex-1 bg-yellow-300/50 h-full border-none cursor-pointer">Send</button>
          </div>
        </div>
      )}
    </div>
